...
exports.postChat = asyncHandler(async (req, res, next) => {
  ...
  if(chatStructure1.blockedContacts.includes(req.body.receiverId) || chatStructure2.blockedContacts.includes(req.user.id)){
    return next(
      new ErrorResponse(`User is blocked from sending or receiving any message from this receiver`, 400)
    );
  }
  ...
  if(!isInChatWith){
    ...
    const update1={userId:req.body.receiverId,name:req.body.receiverName}
...
const doc = await Chats.findOneAndUpdate({_id: req.params.id},{whoShouldSee : [req.body.receiverId]},{new:true})
...
const clearReceivedMsgs = await Chats.updateMany({chatBetween: {$eq : [req.params.receiverId,req.user.id]}},{whoShouldSee : [req.params.receiverId]})
...
exports.clearChats = asyncHandler(async (req, res, next) => {
  ...
  //delete his id from whoShouldSee  from all the chats between this two sent to him, i.e ones he is the receiver
const clearReceivedMsgs = await Chats.updateMany({chatBetween: {$eq : [req.params.receiverId,req.user.id]}},{whoShouldSee : [req.params.receiverId]})
const clearSentMsgs = await Chats.updateMany({chatBetween: {$eq : [req.user.id,req.params.receiverId,]}},{whoShouldSee : [req.params.receiverId]})
...
exports.postNormalFileupload = asyncHandler(async (req, res, next) => {
  ...
  const chats = await Chats.create({
    ...req.body,
    fileName:file.name,
    msg:`${process.env.BACK_END_URL}/uploads/${file.name}`,
