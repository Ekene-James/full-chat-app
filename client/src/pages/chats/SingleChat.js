...
import { addChatsWith } from "../../store/auth/AuthActions";
...
function SingleChat({ submit, socketDeleteMsg, emitStopTyping, emitIsTyping }) {
  ...
  React.useLayoutEffect(() => {
    if (
      ...
      chatCtx?.state?.individualChats[messageReceiver].getFirstLoad
    ) {
  ...
  const handleFileUpload = (e) => {
    ...
    formData.append(
      "receiverName",
      chatCtx?.state?.individualChats[messageReceiver]?.userProfile?.name
    );
    formData.append(
      "isFirstMsg",
  ...
  const handleSubmit = (e) => {
    ...
    submit(msg);
    //check if the two were chatting before, if not, add to lists of my chats

    const isInchatWith =
      authCtx?.state?.home?.user?.chatStructure?.chatsWith.filter(
        (chattingWith) => chattingWith.userId === messageReceiver
      );
    if (isInchatWith?.length === 0) {
      authCtx.dispatch(
        addChatsWith({
          userId: messageReceiver,
          name: chatCtx?.state?.individualChats[messageReceiver]?.userProfile
            ?.name,
        })
      );
    }
      ...
      <div className={classes.container}>
        <div className={classes.msgContainer}>
          {chatCtx?.state?.individualChats[messageReceiver] &&
            ...
            chatCtx?.state?.individualChats[messageReceiver].data.map(
              (msg, i) => (
                <div key={i} ref={scrollRef}>
                  <Messages socketDeleteMsg={socketDeleteMsg} msg={msg} />
                </div>
              )
            )}
        </div>
        ...
        {chatCtx?.state?.individualChats[messageReceiver] &&
          ...
          chatCtx?.state?.individualChats[messageReceiver].isTyping && (
            <div
              className={classes.fileDispalyContainer}
              style={{ background: "green", fontStyle: "italic" }}
            >
              Is Typing...
            </div>
          )}
        {chatFile && (
          <div className={classes.fileDispalyContainer}>
            {chatFile.name}
            {chatFile.size < 1024 * 1024
              ? ` - ${(chatFile.size / 1024).toFixed(2)}KB`
              : ` - ${(chatFile.size / (1024 * 1024)).toFixed(2)}MB`}
            {chatCtx.state.uploadFileProgress
              ? ` - ${chatCtx.state.uploadFileProgress}%`
              : ""}
            <span onClick={handleRemoveFile} style={{ cursor: "pointer" }}>
              <CloseIcon />
            </span>
          </div>
        )}
          ...
          !chatCtx?.state?.individualChats[messageReceiver].chatStructure
            ...
            <form onSubmit={handleSubmit} className={classes.form}>
              ...
              <div className={classes.btnContainer}>
                <div
                  className={classes.emojiCont}
                  style={{ display: openEmoji ? "block" : "none" }}
                >
                  <CloseIcon
