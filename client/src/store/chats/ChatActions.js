...
export const getConversations = async (dispatch) => {
  ...
  try {
    const data = await axios.get(`${backend_server}/api/v1/conversation`);
...
export const getMsgs = async (conversationId, dispatch) => {
  ...
  try {
    const data = await axios.get(
      `${backend_server}/api/v1/message/${conversationId}`
    );
...
export const postChat = async (msg, dispatch) => {
  ...
  try {
    const data = await axios.post(`${backend_server}/api/chats`, msg);
  } catch (error) {
    console.log(error);
...
export const deleteForMe = async (msg, dispatch) => {
  ...
  try {
    const data = await axios.put(
      `${backend_server}/api/chats/singleChat/${msg._id}`,
...
export const userFirstLoad = async (receiverId, dispatch) => {
  ...
  try {
    const data = await axios.get(
      `${backend_server}/api/chats/firstLoadWithUser/${receiverId}`
    );
...
export const blockContact = async (msg, dispatch) => {
  try {
    const data = await axios.put(
      `${backend_server}/api/chatStructure/block/${msg.receiverId}`
    );
...
export const unBlockContact = async (msg, dispatch) => {
  try {
    const data = await axios.put(
      `${backend_server}/api/chatStructure/unblock/${msg.receiverId}`
    );
...
export const clearChats = async (receiverId, dispatch) => {
  ...
  try {
    const data = await axios.put(`${backend_server}/api/chats/${receiverId}`);
...
export const deletChats = async (receiverId, dispatch) => {
  try {
    const data = await axios.delete(
      `${backend_server}/api/chats/${receiverId}`
    );
...
export const uploadNormalFile = async (file, dispatch) => {
  ...
  try {
    const data = await axios.post(`${backend_server}/api/chats/file`, file, {
      headers: {
