...
function Dashboard(props) {
  ...
  React.useMemo(() => {
    setchatWith({
      ...
      subItem: formatData(
        authCtx?.state?.home?.user?.chatStructure?.chatsWith,
        <AccountCircleIcon />,
        "chat",
        authCtx?.state?.user.id,
        "userId"
      ),
    });
  }, [authCtx?.state?.home]);
  ...
  const handleBlockUser = () => {
    ...
    sendBlockToSocket({
      receiverId: authCtx.state.user.id,
      socketReceiverId: messageReceiver,
  ...
  return (
    <Box sx={{ display: "flex" }}>
          ...
          <>
            ...
            <Typography variant="h6" noWrap component="div">
              ...
              chatCtx?.state?.individualChats[messageReceiver].success
                ? chatCtx?.state?.individualChats[messageReceiver].userProfile
                    .name
                : "Welcome"}
            </Typography>
          </>
              ...
              <div>
                ...
                >
                  <img
                    src={
                      chatCtx?.state?.individualChats[messageReceiver]
                        ?.userProfile?.dp || "/img/no-user.jpg"
                    }
                    alt="profile_img"
                    style={{
                      width: "30px",
                ...
                <Menu
                  ...
                  onClose={handleClose}
                >
                  <MenuItem onClick={() => openModal("Profile")}>
                    Profile
                  </MenuItem>
                  {chatCtx?.state?.individualChats[messageReceiver]
                    .chatStructure.youBlocked ? (
                  ...
                  <MenuItem onClick={() => openModal("Clear Chats")}>
                    Clear Chats
                  </MenuItem>
                  {
          ...
          <List>
            ...
            >
              <ListItem
                sx={{
                  cursor: "pointer",
                  "&:hover": { background: "rgba(231, 109, 137,0.6)" },
                }}
                selected={location.pathname === "/chat/profile"}
              >
      ...
      <CustomizedSnackbars
        ...
        setOpen={() =>
          chatCtx.dispatch(
            handleSnackBar({ ...chatCtx.state.snackBarObj, open: false })
          )
        }
      />
      <CustomModal
        show={chatCtx.state.modal.open}
        handleClose={() => {
